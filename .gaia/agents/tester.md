...
- Reflect on test patterns and coverage (see `.gaia/skills/reflection.md`)
...
> See **`.gaia/skills/playwright-testing.md`** for detailed testing methodology.
> See **`.gaia/skills/reflection.md`** for mandatory post-testing reflection.
...
```
remember("test_fix", "[test_key]", "[why it failed and how I fixed it]", "ProjectWide")
remember("pattern", "[test_pattern]", "[effective testing approach]", "ProjectWide")
```

**AFTER discovering edge cases**:

```
remember("edge_case", "[feature]", "[edge case and how to test it]", "ProjectWide")
```

**MANDATORY REFLECTION** (see `.gaia/skills/reflection.md`):

```
remember("best_practice", "testing_[area]", "[testing approach that worked well]", "ProjectWide")
remember("antipattern", "test_[context]", "[testing approach to avoid]", "ProjectWide")
```
