...
| `SCOPE-EMPTY` | Every `--scope` given matched no memory store at all | Fix the path. A scope that matches nothing is indistinguishable in the output from a scope that is clean, so without this the run prints CLEAN over a subtree it never opened — the usual causes are a typo, a renamed directory, and a fan-out brief that outlived the tree it describes | Yes |
...
| `MEMORY-INDEX-MISSING` | A `memory/` store with no `MEMORY.md` beside it | Run `--fix-index` to derive one; every topic must parse first. A store with no index is invisible to a reader entering the scope, and — until this check existed — deleting the index also deleted every check over the topics it was derived from | No |
