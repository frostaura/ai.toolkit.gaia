...
```bash
python3 ${CLAUDE_PLUGIN_ROOT}/scripts/context-audit.py --no-git --fix-index --scope <the directory you are working in>
```
...
The heading is `# MEMORY — <path of the scope relative to the repository root>`; the root's own index uses the repository directory's name. Entries are ordered by type — `alert`, `state`, `decision`, `gotcha`, `question`, `watch`, `kill-record`, `evidence`, `log`, `reference` — with the canonical file of a type (`decisions.md`) before its split siblings (`decisions-sync.md`), then alphabetically. The audit reports `MEMORY-INDEX-STALE` whenever the file on disk differs from that derivation by a byte, and `MEMORY-INDEX-MISSING` when a `memory/` store has no index beside it at all — a store no reader entering the scope can see. `--fix-index` rewrites the first and creates the second, but never invents an index out of nothing: it refuses while any topic in the store has malformed frontmatter, and refuses when the store holds no topic at all (`MEMORY-STORE-EMPTY`), rather than truncating an existing index to a bare heading. **A future `last_verified` counts as malformed frontmatter and blocks creation exactly as it blocks regeneration** — it is the one defect that would otherwise survive the write and then read as freshly verified for as long as its date says. A store with no index is put through every per-topic check *before* anything is written, and reports them on the same run, so an index is never derived from files nothing validated — a non-kebab-case filename (`MEMORY-TOPIC-FILENAME`) would otherwise go straight into every link built from it. **Never hand-edit the index** — a hand-cut hook is the next `MEMORY-INDEX-STALE`, and the next regeneration silently discards it. Only `*.md` topic files live in `memory/`; a `.bak`, a stray note or a subdirectory is `MEMORY-STORE-DEBRIS`.
...
- **`type`** is one of exactly ten, always singular — the *file* may be `decisions.md`, the *type* is `decision`, and a plural defeats any tooling that filters on type: `alert` (red do-not-do-this items — sorted to the top and read at every level passed through), `state` (the scope's current state), `decision` (live decisions with their *why*), `gotcha`, `question` (open questions, naming the owner), `watch` (known drift and unpaid debts), `kill-record`, `evidence` (assembled facts for a pending human-owner call), `log` (a dated session index pruned to pointers, never a changelog), `reference`. That order is the index order, and the list is written in it.
- **`last_verified`** moves only when the topic's claims were re-established by inspection in that session — editing is not verifying. A date in the future is a `MEMORY-FRONTMATTER` finding, and blocks both the regeneration of an index and the creation of a missing one.
...
**Size.** A topic file stays under **60 lines *and* 6,000 characters of body** (`MEMORY-TOPIC-LONG`, `MEMORY-TOPIC-HEAVY`). Both are measured over the *body*: the six frontmatter lines and the blank after them are not content, and counting them left a real ceiling of 53 lines that no one could satisfy by reading the rule. The line rule alone was met with 22 KB files of paragraph-long lines; the character rule closes that. Past either, it is two topics — split by concern (`decisions-sync.md`, `gotchas-build.md`), never by date. A small scope may carry just `state.md` and one or two others; **never pad to a canonical set** — a `watch.md` whose only content is "the schema has never been exercised" is a line in `state.md`, not a file.
...
**Write protocol.** New concern → new topic file, then regenerate the index. Changed concern → edit the topic file, restamp its `last_verified` if you verified it, and regenerate. Dead concern → delete the file and regenerate. **Never hand-edit the index**; a hand-cut hook is the next `MEMORY-INDEX-STALE`. Relative links inside a topic file resolve from `memory/`, one level below the scope — prefix `../`. Topic files carry no `## Upkeep` section (`MEMORY-TOPIC-UPKEEP`) — the write protocol *is* their upkeep, and a "delete this topic once X" condition is the topic's last sentence. **In a fan-out, pass `--scope <your directory>`** so `--fix-index` regenerates only your own indexes and never a sibling's mid-flight. Each `--scope` that matches no memory store is reported on its own as `SCOPE-EMPTY`, so one mis-aimed argument surfaces even when the other scopes are clean — without it a typo printed CLEAN over a subtree nothing opened. And **the scoped command belongs in the `## Upkeep` section of every instruction file whose directory carries a `memory/` store** (`--scope .` at the root): the section is where the next agent reads the command from, so an unscoped one there outlives every brief that got it right (`UPKEEP-UNSCOPED`).
...
3. **Update the memory store at the end of any session that changed reality** — shipped, decided, discovered, abandoned. Edit the topic files, regenerate the index with `--fix-index --scope <this directory>`, and restamp `last_verified` only where you actually checked, not merely edited.
